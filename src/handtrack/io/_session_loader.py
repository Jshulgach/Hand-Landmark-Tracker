...
class SessionLoader:
    ...
    def load_emg(self):
        ...
        if self.verbose:
            print("\n================== EMG =================")
            print("| Searching for EMG data...")
        if not os.path.exists(os.path.join(self.root_dir, "raw", self.label_folder)):
            raise FileNotFoundError(
                f"Raw data folder for label '{self.label}' not found in {self.root_dir}/raw/{self.label_folder}"
            )
        if not os.path.exists(
            os.path.join(
                self.root_dir, "raw", self.label_folder, f"{self.label}_emg_data.npz"
            )
        ):
            raise FileNotFoundError(
                f"EMG data file '{self.label}_emg_data.npz' not found in {self.root_dir}/raw/{self.label_folder}"
            )
        ...
        if self.verbose:
            ...
            print(
                f"|   shape: {emg.shape}, sampling rate: {fs} Hz, time vector: {t.shape}"
            )
    ...
    def load_landmarks(self):
        ...
        landmarks_dir = os.path.join(self.root_dir, "landmarks")
        root_landmarks_path = os.path.join(self.root_dir, landmarks_file)
        nested_landmarks_path = os.path.join(landmarks_dir, landmarks_file)
        if self.verbose:
            print("\n================== Landmarks =================")
            print(f"|  Searching for landmarks in {self.root_dir}")

        if os.path.exists(nested_landmarks_path):
            landmarks_path = nested_landmarks_path
        elif os.path.exists(root_landmarks_path):
            landmarks_path = root_landmarks_path
        else:
            if self.verbose:
                print(
                    f"|  Landmarks file '{landmarks_file}' not found in root directory or landmarks folder."
                )
    ...
    def get_landmarks_from_video(
        ...
        if video_path is None:
            video_path = (
                os.path.join(self.root_dir, "media", f"{self.label}.mp4")
                if self.label
                else os.path.join(self.root_dir, "media", "video.mp4")
            )
    ...
    def load_event_indices(self):
        ...
        indices = extract_event_indices(
            os.path.join(self.root_dir, "events"),
            label=self.label,
    ...
    def load_sync_offset(self):
        ...
        print(
            f"|  Sync offset file not found for label '{self.label}'. Attempting to compute..."
        )
        try:
