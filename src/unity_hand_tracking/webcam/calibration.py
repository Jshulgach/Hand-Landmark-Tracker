"""
calibration.py — Multi-camera ChArUco board calibration using multi_webcam.
...
Uses CameraManager from multi_webcam.py as the camera backend.
...
try:
    ...
    from .multi_webcam import CameraManager
except ImportError:
    from config import (
        ARUCO_DICT,
        CALIBRATION_DIR,
...
def build_grid(frames):
    """Stitch list of frames into a compact grid based on camera count."""
    if not frames:
...
def main():
    ...
    window_name = "Webcam Multi-Camera ChArUco Calibration"
