...
class MultiCameraTracker:
    ...
    def extract_landmarks_2d(self, results):
        ...
        Returns : list of tuples (landmarks_2d,visibilities)
        """
        all_landmarks_2d = []
        ...
        for cam_idx, result in enumerate(results):
            ...
            for hand_landmarks in result.multi_hand_landmarks:
                ...
                visibilities = np.array(
                    [lm.visibility for lm in hand_landmarks.landmark]
                )
                ...
                camera_hands.append((landmarks_undist, visibilities))
    ...
    def match_hands_across_cameras(self, all_landmarks_2d):
        ...
        for hand_idx_0, (landmarks_0, vis_0) in enumerate(all_landmarks_2d[0]):
            group = {0: hand_idx_0}
            ...
            for cam_idx in range(1, self.num_cameras):
                ...
                for hand_idx, (landmarks, vis) in enumerate(all_landmarks_2d[cam_idx]):
    ...
    def triangulate_landmark(
        self, landmark_points_2d, camera_indices, landmark_visibilities=None
    ):
        """
        ...
        landmark_visibilities : optional list of visibility scores
        ...
        if TRIANGULATION_METHOD == "simple_average" or TRIANGULATION_METHOD == "weighted_average":
            triangulated_points = []
            ...
            # Average all triangulated points (weighted if visibility scores provided)
            if landmark_visibilities is not None and TRIANGULATION_METHOD == "weighted_average":
                # Use visibility scores as weights (skip reference camera at index 0)
                weights = np.array(landmark_visibilities[1:])

                # Normalize weights to sum to 1
                weight_sum = np.sum(weights)
                if weight_sum > 0:
                    weights = weights / weight_sum
                    avg_point = np.average(triangulated_points, axis=0, weights=weights)
                else:
    ...
    def triangulate_hand(self, all_landmarks_2d, matched_group):
        ...
        for lm_idx in range(self.num_landmarks):
            ...
            for cam_idx in cam_indices:
                ...
                landmarks, visibilities = all_landmarks_2d[cam_idx][hand_idx]
                landmark_2d = landmarks[lm_idx]
                points_2d.append(landmark_2d)
                landmark_vis.append(visibilities[lm_idx])
                valid_cam_indices.append(cam_idx)

            # Triangulate this landmark
            point_3d = self.triangulate_landmark(points_2d, valid_cam_indices, landmark_vis)
