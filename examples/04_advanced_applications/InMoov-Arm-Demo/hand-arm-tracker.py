...
def compute_triangle(image, point1, point2, point3, visualize=False):
    ...
    return [angle_a, angle_b, angle_c], image

def compute_finger_angles(hand_results, image, handtype="Right"):
    """
    ...
    if hand_results.multi_hand_landmarks:
        for i, hand_landmarks in enumerate(hand_results.multi_hand_landmarks):
            ...
            thumb_angles, image = compute_triangle(image, finger_tips['thumb'], finger_mpc['thumb'], palm[0], True)  # Thumb triangle
            index_angles, image = compute_triangle(image, finger_tips['index'], finger_mpc['index'], palm[0], True)  # Index finger triangle
            middle_angles, image = compute_triangle(image, finger_tips['middle'], finger_mpc['middle'], palm[0], True)  # Middle finger triangle
            ring_angles, image = compute_triangle(image, finger_tips['ring'], finger_mpc['rind'], palm[0], True)  # Ring finger triangle
            pinky_angles, image = compute_triangle(image, finger_tips['pinky'], finger_mpc['pinky'], palm[0], True)  # Pinky finger triangle
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_serial', type=bool, default=False, help='Enable or disable serial commands')
    ...
    while True:
        ...
        if time.time() - start_t > 0.05 and args.use_serial:
            ...
            if finger_angles is not None:
               ...
               #msg = 'set_joints:500,' + str(int(600 + finger_angles[1] - 30)) + ',700,100,300,500'
               wrist_val = '500'
               thumb_val = str(int(700 + 4.0*finger_angles[0]))
               index_val =  str(int(900 - 3.0*finger_angles[1]))
               middle_val = str(int(200 - 2.0*finger_angles[2]))
               ring_val = str(int(200 + 2.0*finger_angles[3]))
               msg = f"set_joints:{wrist_val},{thumb_val},{index_val},{middle_val},{ring_val};\n"
               print(f"Sending: {msg}")
